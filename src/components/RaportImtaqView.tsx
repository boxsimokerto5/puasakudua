...
export const RaportImtaqView: React.FC<RaportImtaqViewProps> = ({
  ...
  return (
    <div className="space-y-3 sm:space-y-4">
      {/* Hero Banner Header (Slim & Compact) */}
      <div className="bg-gradient-to-r from-emerald-950 via-emerald-900 to-slate-950 text-white p-3.5 sm:p-4 rounded-xl sm:rounded-2xl shadow-md border border-emerald-700/50 flex flex-col md:flex-row md:items-center justify-between gap-3 relative overflow-hidden">
        ...
        <div className="flex items-center gap-3 relative z-10">
          <div className="w-10 h-10 sm:w-11 sm:h-11 rounded-xl bg-gradient-to-br from-amber-400 to-amber-600 text-emerald-950 p-2 flex items-center justify-center shadow-md shrink-0">
          ...
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-amber-400 text-emerald-950 shadow-xs">
                Portofolio Imtaq
              </span>
            </div>
            <p className="text-[11px] sm:text-xs text-emerald-200 mt-0.5 line-clamp-1 sm:line-clamp-none">
              Akumulasi riwayat pembiasaan amalan puasa sunnah santri SRT 1 Kediri, predikat ketaqwaan, dan pencetakan piagam resmi.
            </p>
          </div>
        </div>

        {/* Global Batch Action Buttons (Slim) */}
        <div className="flex items-center gap-2 flex-wrap relative z-10 shrink-0">
          <button
            type="button"
            onClick={handlePrintCollectiveReport}
            className="px-3 py-1.5 rounded-lg text-xs font-bold bg-emerald-800/90 hover:bg-emerald-700 text-emerald-100 border border-emerald-600/70 shadow-xs transition-all cursor-pointer flex items-center gap-1.5"
          ...
          <button
            ...
            className="px-3 py-1.5 rounded-lg text-xs font-black bg-gradient-to-r from-amber-400 to-amber-500 hover:from-amber-300 hover:to-amber-400 text-emerald-950 shadow-sm transition-all cursor-pointer flex items-center gap-1.5"
            title="Cetak Piagam Santri Istiqomah sekaligus"
          >
            <Medal className="w-3.5 h-3.5" />
            <span>Semua Piagam ({eligibleStudents.length})</span>
          </button>
          ...
          {onLogout && (
            <button
              type="button"
              onClick={onLogout}
              className="px-2.5 py-1.5 rounded-lg text-xs font-bold bg-red-600 hover:bg-red-700 active:scale-95 text-white border border-red-400 shadow-xs transition-all cursor-pointer flex items-center gap-1"
      ...
      {/* Summary KPI Statistic Cards (Slim & Dense) */}
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2 sm:gap-2.5">
        {/* Card 1: Total Sesi */}
        <div className="bg-white p-2.5 sm:p-3 rounded-xl border border-gray-200 shadow-2xs flex flex-col justify-between">
          <div className="flex items-center justify-between text-gray-500">
            <span className="text-[10px] font-bold uppercase tracking-tight">Total Sesi</span>
            <Calendar className="w-3.5 h-3.5 text-emerald-600" />
          </div>
          <div className="mt-1">
            <div className="text-lg sm:text-xl font-black text-gray-900 leading-tight">{globalSummary.totalSessions}</div>
            <div className="text-[10px] text-gray-500 font-medium">Kegiatan Terjadwal</div>
          </div>
        </div>
        ...
        <div className="bg-white p-2.5 sm:p-3 rounded-xl border border-gray-200 shadow-2xs flex flex-col justify-between">
          <div className="flex items-center justify-between text-gray-500">
            <span className="text-[10px] font-bold uppercase tracking-tight">Rata-rata</span>
            <Sparkles className="w-3.5 h-3.5 text-amber-500" />
          </div>
          <div className="mt-1">
            <div className="text-lg sm:text-xl font-black text-emerald-700 leading-tight">{globalSummary.avgPercentage}%</div>
            <div className="text-[10px] text-gray-500 font-medium">Keistiqomahan</div>
          </div>
        </div>

        {/* Card 3: Mumtaz (A) */}
        <div className="bg-emerald-50/70 p-2.5 sm:p-3 rounded-xl border border-emerald-200 shadow-2xs flex flex-col justify-between">
          <div className="flex items-center justify-between text-emerald-700">
            <span className="text-[10px] font-bold uppercase tracking-tight">Mumtaz (A)</span>
            <Trophy className="w-3.5 h-3.5 text-emerald-600" />
          </div>
          <div className="mt-1">
            <div className="text-lg sm:text-xl font-black text-emerald-900 leading-tight">{globalSummary.mumtazCount}</div>
            <div className="text-[10px] text-emerald-700 font-medium">&ge;85%</div>
          </div>
        </div>

        {/* Card 4: Jayyid Jiddan (B+) */}
        <div className="bg-sky-50/70 p-2.5 sm:p-3 rounded-xl border border-sky-200 shadow-2xs flex flex-col justify-between">
          <div className="flex items-center justify-between text-sky-700">
            <span className="text-[10px] font-bold uppercase tracking-tight">Jayyid Jiddan</span>
            <Medal className="w-3.5 h-3.5 text-sky-600" />
          </div>
          <div className="mt-1">
            <div className="text-lg sm:text-xl font-black text-sky-900 leading-tight">{globalSummary.jayyidJiddanCount}</div>
            <div className="text-[10px] text-sky-700 font-medium">70% - 84%</div>
          </div>
        </div>

        {/* Card 5: Jayyid (B) */}
        <div className="bg-amber-50/70 p-2.5 sm:p-3 rounded-xl border border-amber-200 shadow-2xs flex flex-col justify-between">
          <div className="flex items-center justify-between text-amber-700">
            <span className="text-[10px] font-bold uppercase tracking-tight">Jayyid (B)</span>
            <Star className="w-3.5 h-3.5 text-amber-600" />
          </div>
          <div className="mt-1">
            <div className="text-lg sm:text-xl font-black text-amber-900 leading-tight">{globalSummary.jayyidCount}</div>
            <div className="text-[10px] text-amber-700 font-medium">50% - 69%</div>
          </div>
        </div>

        {/* Card 6: Berhak Piagam */}
        <div className="bg-gradient-to-br from-amber-400 to-amber-500 text-emerald-950 p-2.5 sm:p-3 rounded-xl shadow-2xs flex flex-col justify-between">
          <div className="flex items-center justify-between text-emerald-950/80">
            <span className="text-[10px] font-extrabold uppercase tracking-tight">Berhak Piagam</span>
            <Award className="w-3.5 h-3.5 text-emerald-950" />
          </div>
          <div className="mt-1">
            <div className="text-lg sm:text-xl font-black text-emerald-950 leading-tight">{globalSummary.eligibleCertCount}</div>
            <div className="text-[10px] text-emerald-950/80 font-bold">Santri Layak</div>
          </div>
        </div>
      </div>

      {/* Filter and Search Bar (Compact & Slim) */}
      <div className="bg-white p-2.5 sm:p-3 rounded-xl border border-gray-200 shadow-2xs flex flex-col sm:flex-row sm:items-center justify-between gap-2.5">
        {/* Left Filter Controls */}
        <div className="flex items-center gap-2 flex-wrap">
          {/* Level Filter */}
          <div className="flex items-center gap-1.5 bg-gray-50 px-2 py-1 rounded-lg border border-gray-200 text-xs">
            <GraduationCap className="w-3.5 h-3.5 text-gray-500" />
            <span className="font-semibold text-gray-600 text-[11px]">Jenjang:</span>
            <select
              value={selectedLevel}
          ...
          <div className="flex items-center gap-1.5 bg-gray-50 px-2 py-1 rounded-lg border border-gray-200 text-xs">
            ...
            <span className="font-semibold text-gray-600 text-[11px]">Kelas:</span>
            <select
              value={selectedClass}
          ...
          <div className="flex items-center gap-1.5 bg-gray-50 px-2 py-1 rounded-lg border border-gray-200 text-xs">
            ...
            <span className="font-semibold text-gray-600 text-[11px]">Predikat:</span>
            <select
        ...
        <div className="flex items-center gap-2">
          <div className="relative flex-1 sm:w-56">
            ...
            <input
              ...
              className="w-full pl-8 pr-2.5 py-1 text-xs bg-gray-50 border border-gray-200 rounded-lg focus:outline-none focus:ring-1.5 focus:ring-emerald-600"
            />
            {searchQuery && (
              <button
                type="button"
          ...
          <div className="flex items-center bg-gray-100 p-0.5 rounded-lg border border-gray-200">
            <button
              ...
              className={`px-2 py-0.5 rounded-md text-xs font-bold transition-all cursor-pointer ${
                viewMode === 'table' ? 'bg-white text-emerald-950 shadow-2xs' : 'text-gray-500 hover:text-gray-900'
              }`}
            >
              Tabel
            </button>
            <button
              ...
              className={`px-2 py-0.5 rounded-md text-xs font-bold transition-all cursor-pointer ${
                viewMode === 'grid' ? 'bg-white text-emerald-950 shadow-2xs' : 'text-gray-500 hover:text-gray-900'
              }`}
            >
              Kartu
            </button>
            ...
            <table className="w-full min-w-[800px] text-left border-collapse">
              <thead>
                <tr className="bg-emerald-950 text-white text-[11px] font-bold uppercase tracking-wider">
                  ...
                  <th className="py-2.5 px-3">Kelas & Jenjang</th>
                  <th className="py-2.5 px-2 text-center">L/P</th>
                  <th className="py-2.5 px-3 text-center">Hari Puasa</th>
                  <th className="py-2.5 px-3 text-center">Udzur</th>
                  <th className="py-2.5 px-3">Tingkat Istiqomah</th>
                  <th className="py-2.5 px-3 text-center">Predikat</th>
                  <th className="py-2.5 px-3 text-center">Aksi Dokumen</th>
                </tr>
              </thead>
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="py-2 px-2.5 text-center font-bold">
                        {isTop3 ? (
                          <span
                            className={`inline-flex items-center justify-center w-5 h-5 rounded-full text-[10px] font-black text-white shadow-2xs ${
                              index === 0
                                ? 'bg-amber-500 ring-1 ring-amber-300'
                                : index === 1
                                ? 'bg-slate-400 ring-1 ring-slate-300'
                                : 'bg-amber-700 ring-1 ring-amber-600'
                            }`}
                          >
                            {index + 1}
                          </span>
                      ...
                      <td className="py-2 px-3">
                        <div className="font-bold text-gray-900 group-hover:text-emerald-800 transition-colors leading-tight">
                          {s.nama}
                        </div>
                      ...
                      {/* Gender */}
                      <td className="py-2 px-2 text-center font-bold text-gray-500 text-[11px]">
                      ...
                      <td className="py-2 px-3 text-center">
                        ...
                        <span className="text-gray-400 text-[10px]">/ {item.totalSessions} hari</span>
                      </td>
                      ...
                      >
                        <div className="flex items-center justify-center gap-1">
                          <button
                            type="button"
                            onClick={() => setSelectedStudentStats(item)}
                            className="p-1 text-emerald-800 hover:bg-emerald-100 rounded transition-colors cursor-pointer"
                          ...
                          <button
                            ...
                            className="px-1.5 py-0.5 text-[10px] font-bold bg-emerald-50 hover:bg-emerald-100 text-emerald-900 border border-emerald-300 rounded transition-colors cursor-pointer flex items-center gap-0.5"
                          ...
                          {item.isEligibleForCertificate && (
                            <button
                              ...
                              className="px-1.5 py-0.5 text-[10px] font-bold bg-amber-100 hover:bg-amber-200 text-amber-900 border border-amber-300 rounded transition-colors cursor-pointer flex items-center gap-0.5"
          ...
          {filteredStats.map((item, index) => {
            ...
            return (
              <div
                key={s.id}
                onClick={() => setSelectedStudentStats(item)}
                className="bg-white p-3.5 rounded-xl border border-gray-200 hover:border-emerald-500 shadow-2xs hover:shadow-sm transition-all cursor-pointer flex flex-col justify-between group"
              >
                <div>
                  {/* Top Bar inside Card */}
                  ...
                  {/* Student Name */}
                  <h4 className="text-xs sm:text-sm font-bold text-gray-900 group-hover:text-emerald-800 transition-colors leading-tight">
                    {s.nama}
                  </h4>
                  <p className="text-[10px] text-gray-400 mt-0.5">NIK: {s.nik || s.no}</p>

                  {/* Stats Mini Grid */}
                  <div className="grid grid-cols-3 gap-1.5 mt-2.5 p-2 bg-gray-50 rounded-lg border border-gray-100 text-center">
                    <div>
                      <span className="text-[9px] text-gray-400 block uppercase font-bold">Puasa</span>
                      <span className="text-xs font-black text-emerald-700">
                        {item.berpuasaCount}
                      </span>
                      <span className="text-[9px] text-gray-400">/{item.totalSessions}</span>
                    </div>
                    <div>
                      <span className="text-[9px] text-gray-400 block uppercase font-bold">Udzur</span>
                      <span className="text-xs font-black text-rose-600">
                        {item.halanganCount + item.tidakPuasaCount}
                      </span>
                    </div>
                    <div>
                      <span className="text-[9px] text-gray-400 block uppercase font-bold">Konsistensi</span>
                ...
                {/* Card Action Buttons */}
                <div
                  className="mt-2.5 pt-2 border-t border-gray-100 flex items-center justify-between gap-1.5"
                  onClick={(e) => e.stopPropagation()}
                >
                  <button
                    type="button"
                    onClick={() => generateStudentImtaqReportPdf(item, pembinaName, kepalaName)}
                    className="flex-1 py-1 px-1.5 bg-emerald-50 hover:bg-emerald-100 text-emerald-900 border border-emerald-200 rounded text-[10px] font-bold flex items-center justify-center gap-1 transition-colors cursor-pointer"
                  >
                    <Download className="w-2.5 h-2.5 text-emerald-700" />
                  ...
                  {item.isEligibleForCertificate && (
                    <button
                      type="button"
                      onClick={() => generateImtaqCertificatePdf(item, pembinaName, kepalaName)}
                      className="flex-1 py-1 px-1.5 bg-amber-100 hover:bg-amber-200 text-amber-950 border border-amber-300 rounded text-[10px] font-bold flex items-center justify-center gap-1 transition-colors cursor-pointer"
                    >
                      <Medal className="w-2.5 h-2.5 text-amber-700" />
      ...
      {selectedStudentStats && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-xs flex items-center justify-center p-3 sm:p-4 z-50 animate-fade-in">
          <div className="bg-white w-full max-w-2xl rounded-2xl shadow-2xl border border-gray-200 overflow-hidden flex flex-col max-h-[90vh]">
            {/* Modal Header (Slim) */}
            <div className="bg-gradient-to-r from-emerald-950 to-emerald-900 text-white p-3.5 sm:p-4 flex items-center justify-between">
            ...
            {/* Modal Body (Compact & Dense) */}
            <div className="p-4 overflow-y-auto space-y-3 flex-1 text-xs custom-scrollbar">
              {/* Profile Card & Predicate */}
              <div className="bg-gray-50 p-3 rounded-xl border border-gray-200 flex flex-col sm:flex-row sm:items-center justify-between gap-2.5">
                <div>
                  <h4 className="text-sm font-black text-gray-900">
                    {selectedStudentStats.student.nama}
                  </h4>
                  <div className="flex items-center gap-2 text-gray-500 mt-0.5 text-[11px] font-medium flex-wrap">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-center">
                <div className="p-2 bg-emerald-50 rounded-lg border border-emerald-200">
                  <span className="text-[9px] font-bold uppercase text-emerald-800 block">Hari Puasa</span>
                ...
                <div className="p-2 bg-sky-50 rounded-lg border border-sky-200">
                  <span className="text-[9px] font-bold uppercase text-sky-800 block">Konsistensi</span>
                  <span className="text-base font-black text-sky-900">
                    {selectedStudentStats.percentage}%
                  </span>
                  <span className="text-[9px] text-sky-700 block">
                    Tingkat Kehadiran
                  </span>
                </div>

                <div className="p-2 bg-rose-50 rounded-lg border border-rose-200">
                  <span className="text-[9px] font-bold uppercase text-rose-800 block">Udzur/Halangan</span>
                  ...
                  <span className="text-[9px] text-rose-700 block">
                    {selectedStudentStats.halanganCount} Udzur, {selectedStudentStats.tidakPuasaCount} Izin
                  </span>
                </div>

                <div className="p-2 bg-amber-50 rounded-lg border border-amber-200">
                  <span className="text-[9px] font-bold uppercase text-amber-800 block">Rekor Istiqomah</span>
              ...
              <div>
                <h5 className="font-bold text-gray-900 mb-1.5 flex items-center gap-1 text-xs">
                  <Calendar className="w-3.5 h-3.5 text-emerald-700" />
                  <span>Riwayat Presensi Per Sesi Kegiatan Puasa</span>
                </h5>
                <div className="border border-gray-200 rounded-lg overflow-hidden max-h-48 overflow-y-auto custom-scrollbar">
                  <table className="w-full text-left border-collapse">
                    <thead className="sticky top-0 bg-gray-100 z-10">
                      <tr className="text-gray-700 text-[10px] uppercase font-bold">
                        <th className="py-1.5 px-2 text-center w-8">No</th>
                        <th className="py-1.5 px-2">Tanggal & Sesi</th>
                        <th className="py-1.5 px-2 text-center">Status Amalan</th>
                    ...
                    <tbody className="divide-y divide-gray-100 text-[10px]">
                      {selectedStudentStats.history.map((h, i) => (
                        <tr key={h.sessionId} className="hover:bg-gray-50">
                          <td className="py-1.5 px-2 text-center font-mono text-gray-400">{i + 1}</td>
                          ...
                          <td className="py-1.5 px-2 text-center">
                            {h.status === 'berpuasa' ? (
                              ...
                              <span className="px-1.5 py-0.5 rounded-full text-[9px] font-bold bg-amber-100 text-amber-900 border border-amber-300">
                                Halangan
                              </span>
                            ) : h.status === 'tidak_puasa' ? (
                              <span className="px-1.5 py-0.5 rounded-full text-[9px] font-bold bg-rose-100 text-rose-900 border border-rose-300">
                                Tidak Puasa
                              </span>
                            ) : (
                          ...
                          <td className="py-1.5 px-2 text-gray-500 italic">{h.notes || '-'}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-2 py-1 text-xs bg-white border border-gray-300 rounded focus:outline-none focus:ring-1 focus:ring-emerald-600"
                    />
                  </div>
                  <div>
                    <label className="text-[10px] text-gray-500 font-semibold block mb-0.5">
            ...
            <div className="bg-gray-50 p-3 border-t border-gray-200 flex flex-wrap items-center justify-between gap-2">
              <button
                type="button"
                onClick={() => setSelectedStudentStats(null)}
                className="px-3 py-1.5 text-xs font-semibold text-gray-600 hover:bg-gray-200 rounded-lg transition-colors cursor-pointer"
              >
                Tutup
              </button>
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="px-3 py-1.5 bg-emerald-800 hover:bg-emerald-900 text-white rounded-lg text-xs font-bold flex items-center gap-1.5 shadow-2xs transition-all cursor-pointer"
                >
                ...
                {selectedStudentStats.isEligibleForCertificate && (
                  <button
                    ...
                    className="px-3 py-1.5 bg-gradient-to-r from-amber-400 to-amber-500 hover:from-amber-300 hover:to-amber-400 text-emerald-950 rounded-lg text-xs font-black flex items-center gap-1.5 shadow-xs transition-all cursor-pointer"
                  >
                    <Medal className="w-3.5 h-3.5" />
